...
local function lsp_highlight_document(client)
  if settings.highlight == false then return end
...
local function lsp_buffer_keybindings(bufnr)
  ...
  local leader_keys = {
    ...
    ["l"] = {
      ...
      ["R"] = {"<cmd>lua vim.lsp.buf.rename()<CR>", "Rename symbol projectwise"},
      ["I"] = {
...
function M.common_on_init(client, bufnr)
  ...
  if not vim.tbl_isempty(formatters) and formatters[1]["exe"] ~= nil and
      formatters[1].exe ~= "" then
...
local function is_client_active(name)
  ...
  for _, client in pairs(clients) do
    if client.name == name then return true, client end
  end
  return false
end
...
function M.setup(lang)
  ...
  if is_client_active(lsp.provider) then return end
